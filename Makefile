...
smoke: ## Every subcommand answers --help without importing the world
	./scripts/rt help >/dev/null
	./scripts/rt version
	@for c in live trace debt context cache quality horizon policy outcomes \
	          classify savings verify validate regret simulate ab; do \
	  ./scripts/rt $$c --help >/dev/null || { echo "FAIL: rt $$c --help"; exit 1; }; \
	done
...
print('wheel:', w); \
missing=[p for p in ('router/cli.py','router/py.typed') if p not in n]; \
...
release-check: check verify-dist ## Everything that must pass before tagging a release
	@$(PY) -c "import router; print('version:', router.__version__)"
