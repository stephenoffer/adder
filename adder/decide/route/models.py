...
def cmd_show(cat: Catalog, a: argparse.Namespace) -> int:
    ...
    if e.elo:
        for board, v in sorted(e.elo.items(), key=lambda kv: -kv[1]):
            lo, hi = e.elo_lo.get(board), e.elo_hi.get(board)
            # The interval is the difference between a rating and a claim. A
            # 17-point lead between two overlapping bars is not a lead, and a
            # reader cannot tell without seeing them.
            span = f"  95% [{lo:,.0f}, {hi:,.0f}]" if lo is not None and hi is not None else ""
            print(f"  elo        {v:,.0f}{span}  ({board}, {e.votes:,} votes)")
        if e.rating_variant:
            print(f"  rated as   {e.rating_variant}"
                  + ("   (a reasoning-effort variant; the price above is the "
                     "model's default)" if not e.rating_variant.endswith(e.key) else ""))
...
def _reference_for(h) -> str:
    """The "do it properly" model to compare a ladder against.

    Anchored on the harness rather than hardcoded, because the reference has to
    be a model the harness can actually run as its main session. Comparing a
    Codex ladder against Claude Opus measures the gap to a placement that user
    does not have.
    """
    from adder.core import settings as _settings
    from adder.pricing.registry import candidates

    # The configured session model, not a literal. A harness that routes freely
    # has no vendor to anchor on, so the honest anchor is the model this machine
    # says it runs -- which is the whole point of the `model` setting.
    fallback = _settings.session_model()
    if not h.pins_main_session:
        return fallback
    best = candidates(org=h.main_session_org, needs_tools=True, priced_only=True)
    return best[-1].id if best else fallback
...
def cmd_ladder(cat: Catalog, a: argparse.Namespace) -> int:
    ...
    for rung, ceiling, purpose in BANDS:
        ...
        cands = [
            ...
            and h.allows_main_session(e.org)
        ]
    ...
    for w in ladder_warnings():
        print(f"\n!  {w}")

    if ref is not None and not h.pins_main_session:
        c = cost_of(ref, need)
...
def cmd_refresh(a: argparse.Namespace) -> int:
    ...
    """
    from adder.pricing.sources import refresh
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_lad.add_argument("--harness", default=_harness_default(),
                       choices=_harness_names(),
                       help="agent runtime; pinning harnesses restrict the "
                            "ladder to their own vendor")
