...
`adder.measure.session.quality` and refuses to claim a saving when any of them regressed.
"""
...
def compare(cutover: date, root: Path | str = DEFAULT_ROOT) -> tuple[Window, Window]:
    ...
    for s in load_sessions(root).values():
        ...
        # cumulative-output model of how a prefix grows, and a subagent's turns
        # are not in that prefix. Counting them dragged the median context down
...
def report(cutover: date, root: Path | str = DEFAULT_ROOT) -> str:
    ...
    try:
        from adder.measure.session.quality import compare as qcompare
        from adder.measure.session.quality import regressions
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(prog="adder verify",
                                 description="Measure whether an intervention landed.")
    ap.add_argument("--since", required=True,
                    help="cutover date: YYYY-MM-DD, 7d, 2w, today")
    ...
    cutover = parse_date(a.since)
    if a.json:
        import json

        b, aft = compare(cutover, a.root)
        print(json.dumps({
            "cutover": cutover.isoformat(),
            "windows": {
                w.label: {"turns": w.turns, "sessions": w.sessions,
                          "cost": round(w.cost, 4),
                          "cost_per_turn": round(w.cost_per_turn, 6),
                          "out_per_turn": round(w.out_per_turn, 1),
                          "median_context": round(w.median_ctx, 1)}
                for w in (b, aft)
            },
        }))
        return 0
