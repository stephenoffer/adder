"""Single entry point for every `rt` subcommand.
...
Lazy import matters: `rt live` should not pay to import the A/B harness.
"""
...
COMMANDS: tuple[Command, ...] = (
    ...
    Command("live", "router.live", "Measure", "[--cwd DIR]",
            "this session: cost/turn, next-turn cost, pressure"),
    Command("trace", "router.trace", "Measure", "[root] [--json] [--verify]",
            "total spend, by model and session"),
    Command("debt", "router.debt", "Measure", "[root]",
            "what an output token really costs"),
    Command("context", "router.context", "Measure", "[root]",
            "where context growth comes from"),
    Command("cache", "router.cache", "Measure", "[root]",
            "cache hit rate and rebuild waste, by cause"),
    Command("quality", "router.quality", "Measure", "[root] [--since DATE]",
            "agent-performance proxies"),
    Command("horizon", "router.horizon", "Measure", "[root]",
            "remaining-turns estimate vs the naive countdown"),
    # Decide — turn a measurement into a routing choice.
    Command("policy", "router.policy", "Decide", '"<task>" [--json]',
            "route a task: inline vs delegate"),
    Command("outcomes", "router.outcomes", "Decide", "[--log PATH]",
            "escalation calibration (p_fail)"),
    Command("classify", "router.classify", "Decide", '"<task>"',
            "task-complexity classification, on its own"),
    Command("pick", "router.select", "Decide", '"<task>" [--combos] [--json]',
            "cheapest model, or combination, that clears the quality bar"),
    Command("models", "router.models", "Decide", "[list|show|ladder|refresh]",
            "the cross-provider catalog: what exists, at what price and rating"),
    # Evaluate — check that a lever is real before trusting it.
    Command("savings", "router.savings", "Evaluate", "[root] [--max-turns N]",
            "what each lever is worth"),
    Command("verify", "router.verify", "Evaluate", "--since DATE [root]",
            "did a change actually land?"),
    Command("validate", "router.validate", "Evaluate", "[root]",
            "re-test the claims everything rests on"),
    Command("regret", "router.regret", "Evaluate", "[root]",
            "dollar regret of the horizon estimator"),
    Command("simulate", "router.simulate", "Evaluate", "[root]",
            "replay sessions under interventions; test lever composition"),
    Command("ab", "router.ab", "Evaluate", "--help",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv or argv[0] in ("help", "-h", "--help"):
        # `rt help <command>` forwards to that command's own parser.
