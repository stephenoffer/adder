...
`rt models` is the answer to "what can this thing route to today". Four views:

    rt models                what is in the catalog, by value
    rt models refresh        pull the public sources (the only networked command)
    rt models show <name>    everything known about one model
    rt models ladder         the hardcoded tier ladder vs what the data says

`ladder` is the one that matters over time. The router ships a three-rung
...
a new model lands, prices move, an intro rate expires, and the router keeps
...
def cmd_list(cat: Catalog, a: argparse.Namespace) -> int:
    ...
    if cat.is_stale():
        print("  ! stale; run `rt models refresh`")
    print()
...
def cmd_ladder(cat: Catalog, a: argparse.Namespace) -> int:
    ...
    if drift:
        ...
        print("  survives `rt savings` on your own history.")
    else:
...
def cmd_refresh(a: argparse.Namespace) -> int:
    ...
    Imported here rather than at module scope so that `rt models list` -- and
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="router.models",
