...
* **Opt-in.** Nothing calls `refresh()` implicitly. `LLM_ROUTER_OFFLINE=1`
...
* **Reproducible offline.** Every fetcher has a matching parser that takes raw
  bytes, so `rt models refresh --from lmarena=page.html` replays a saved
...
USER_AGENT = "llm-router/0.2 (+https://github.com/; catalog refresh)"
TIMEOUT = 30
...
def fetch(url: str, *, timeout: int = TIMEOUT) -> bytes:
    ...
    if os.environ.get("LLM_ROUTER_OFFLINE"):
        raise Offline("LLM_ROUTER_OFFLINE is set; refusing to fetch " + url)
