...
from adder.evaluate.replay.ab import TASKS, ArmResult, Outcome, report, wilson_lower_bound
...
class TestTheWilsonBoundIsTheSharedOne:
    """`util.stats` opens by saying three modules disagreed about a quantile.

    This module carried an eighth private transcription of the Wilson bound,
    and it had already drifted: no clamp on an out-of-range count (a `passed`
    above `n` raised a math domain error) and no snap on the exact endpoints
    (a clean run reported 2.8e-17 where every other report prints 0).
    """

    @pytest.mark.parametrize("k,n", [(0, 0), (0, 12), (12, 12), (7, 10), (3, 4)])
    def test_it_agrees_with_stats(self, k, n):
        from adder.evaluate.replay.ab import wilson_lower_bound
        from adder.util.stats import wilson_interval

        assert wilson_lower_bound(k, n) == wilson_interval(k, n)[0]

    def test_a_perfect_run_is_not_a_certainty(self):
        from adder.evaluate.replay.ab import wilson_lower_bound

        assert 0.0 < wilson_lower_bound(12, 12) < 1.0

    def test_a_clean_zero_is_exactly_zero(self):
        from adder.evaluate.replay.ab import wilson_lower_bound

        assert wilson_lower_bound(0, 40) == 0.0

    @pytest.mark.parametrize("k,n", [(15, 10), (-1, 5)])
    def test_an_out_of_range_count_does_not_raise(self, k, n):
        from adder.evaluate.replay.ab import wilson_lower_bound

        assert 0.0 <= wilson_lower_bound(k, n) <= 1.0
