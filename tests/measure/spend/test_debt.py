...
from adder.measure.spend.debt import (
...
class TestTheIrreducibleBaselineIsPerChain:
    """Each chain is charged its own floor for its own turns.

    `Session.base_context` is the MAIN chain's floor -- system prompt, tools,
    CLAUDE.md. Multiplying it by every turn in the session charged each
    delegated turn for a prefix it never carried, which overstates the
    irreducible baseline and so understates `accumulated`: the pool every
    substitute lever in `savings` is scaled by. `base_context`'s own docstring
    spells the same arithmetic out in the other direction.
    """
    ...
    def _session(self):
        ...
        def turn(i, ctx, *, side=False, model="claude-opus-5"):
            return Turn("s", "p", model, 0, ctx, 0, 100, 0, side,
                        ts=(start + timedelta(minutes=i)).isoformat())

        s = Session("s", "p")
        s.turns = [turn(i, 100_000 + 2_000 * i) for i in range(40)]
        s.turns += [turn(40 + i, 15_000, side=True, model="claude-haiku-4-5")
                    for i in range(40)]
        return {"s": s}

    def test_the_baseline_never_exceeds_what_was_spent(self):
        from adder.measure.spend.debt import decompose_read_cost

        total, baseline, _ = decompose_read_cost(self._session())
        assert baseline <= total

    def test_the_pool_is_not_swallowed_by_a_floor_the_subagents_never_paid(self):
        from adder.measure.spend.debt import decompose_read_cost

        sessions = self._session()
        _, _, accumulated = decompose_read_cost(sessions)
        s = sessions["s"]
        # The old arithmetic: main-chain floor x every turn in the session.
        old_baseline = (s.base_context * len(s.turns)
                        * s.main_turns[0].rates().cache_read / 1_000_000)
        measured = sum(t.cache_read * t.rates().cache_read / 1_000_000
                       for t in s.turns)
        assert accumulated > max(0.0, measured - old_baseline)

    def test_the_three_parts_still_reconcile(self):
        from adder.measure.spend.debt import decompose_read_cost

        total, baseline, accumulated = decompose_read_cost(self._session())
        assert baseline + accumulated == pytest.approx(total)
    ...
    def test_a_session_with_no_delegation_is_unchanged(self):
        ...
        start = datetime(2026, 8, 1, 9, 0, tzinfo=timezone.utc)
        s = Session("s", "p")
        s.turns = [Turn("s", "p", "claude-opus-5", 0, 100_000 + 2_000 * i, 0,
                        100, 0, False,
                        ts=(start + timedelta(minutes=i)).isoformat())
                   for i in range(40)]
        total, baseline, _ = decompose_read_cost({"s": s})
        expected = (s.base_context * len(s.turns)
                    * s.turns[0].rates().cache_read / 1_000_000)
        assert baseline == pytest.approx(min(expected, total))
