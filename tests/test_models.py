...
class TestList:
    ...
    def test_the_generated_claude_layer_does_not_hide_staleness(self, tmp_path,
        ...
        from adder.catalog import load
...
class TestLadder:
    ...
    def test_drift_is_reported_not_applied(self, tmp_path, monkeypatch, capsys):
        ...
        from adder.classify import LADDER, Tier
...
class TestRefresh:
    def test_replays_captures_without_a_socket(self, tmp_path, monkeypatch, capsys):
        from test_sources import ARENA_PAGE, OPENROUTER_PAGE
    ...
    def test_if_stale_does_refresh_once_the_catalog_ages_out(self, tmp_path,
                                                             monkeypatch, capsys):
        from test_sources import OPENROUTER_PAGE
    ...
    def test_refresh_is_the_only_networked_path(self):
        ...
        tree = ast.parse(pathlib.Path("adder/models.py").read_text(encoding="utf-8"))
        top = [n for n in tree.body if isinstance(n, (ast.Import, ast.ImportFrom))]
        names = [getattr(n, "module", "") or "" for n in top]
        assert "adder.sources" not in names and "sources" not in names
